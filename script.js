...
function selectWinner() {
    ...
    if (checkIdSign(1, 2, 3, playerSign) || checkIdSign(4, 5, 6, playerSign) ||
        ...
        // If there is a winner, stop the bot and show the result
        runBot = false;
        bot(runBot); // Make sure bot is stopped from making any moves
        ...
        // Check for a draw if all boxes are filled and there is no winner
        let allBoxesFilled = true;
        for (let i = 0; i < allBox.length; i++) {
            if (allBox[i].id === "") {
                allBoxesFilled = false;
                break; // If any box is empty, stop checking further
            }
        }
        ...
        if (allBoxesFilled) {
            runBot = false;
            bot(runBot); // Stop the bot

            setTimeout(() => {
                resultBox.classList.add("show");
                playBoard.classList.remove("show");
            }, 700);
            wonText.textContent = "Match has been drawn!"; // Display draw message
        }
    }
}
